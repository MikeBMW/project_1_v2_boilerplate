...
## Below is the changing comment by Udacity Reviews

bolck.js
line 38
This is not the correct way to implement validate() function. I will list down the steps on how to validate a block.

    Store the current hash of the block in a variable. let hashAux = self.hash.
    Make the hash of the block as null. self.hash = null.
    Calculate the hash value again and store it in a different variable. let calculatedHash = SHA256(JSON.stringify(self)).toString()
    Assign the original hash value to the hash property of the block. self.hash = hashAux.
    Compare the hashAux and calculatedHash.

You are only missing the one step from above in order to complete this function. After calculating the hash value again, you need to store the original hash value back in the hash property of the block.

You need to write the below code after line 43
...
blockchain.js
line 217
This line of code is throwing an error because chain array inside a function can only be accessed using self. So, you need to use self.chain instead of chain here.
